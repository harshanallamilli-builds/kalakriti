...
export async function signUp(
  ...
  const state = sanitizeText(formData.get("state"));
  const whatsapp = sanitizeText(formData.get("whatsapp"));
  const linksRaw = sanitizeText(formData.get("links"));
  let parsedLinks: { label: string; url: string }[] = [];
  try { parsedLinks = linksRaw ? JSON.parse(linksRaw) : []; } catch { parsedLinks = []; }
  ...
  const { error: profileError } = await supabase.from("profiles").insert({
    ...
    city: city || null,
    state: state || null,
    links: role === "creator" ? parsedLinks : [],
  });
...
export async function updateProfile(
  ...
  if (isCreator) {
    ...
    updates.craft = sanitizeText(formData.get("craft")) || null;
    updates.store_name = sanitizeText(formData.get("store_name")) || null;
    const linksRaw2 = sanitizeText(formData.get("links"));
    let parsedLinks2: { label: string; url: string }[] = [];
    try { parsedLinks2 = linksRaw2 ? JSON.parse(linksRaw2) : []; } catch { parsedLinks2 = []; }

    // Also save the legacy individual URL fields so they persist in the DB
    const igUrl = sanitizeText(formData.get("instagram_url")) || null;
    const webUrl = sanitizeText(formData.get("website_url")) || null;
    const ytUrl = sanitizeText(formData.get("youtube_url")) || null;
    updates.instagram_url = igUrl;
    updates.website_url = webUrl;
    updates.youtube_url = ytUrl;

    // Merge legacy URL fields into the links array (avoid duplicates)
    const existingLabels = new Set(parsedLinks2.map((l) => l.label.toLowerCase()));
    if (igUrl && !existingLabels.has("instagram")) parsedLinks2.push({ label: "Instagram", url: igUrl });
    if (webUrl && !existingLabels.has("website"))  parsedLinks2.push({ label: "Website",   url: webUrl });
    if (ytUrl && !existingLabels.has("youtube"))   parsedLinks2.push({ label: "YouTube",   url: ytUrl });

    updates.links = parsedLinks2;
    const yearsRaw = formData.get("years_experience");
    updates.years_experience = yearsRaw ? parseInt(String(yearsRaw), 10) || null : null;
  }
...
export async function completeCreatorSetup(
  ...
  let parsedLinks: { label: string; url: string }[] = [];
  try { parsedLinks = linksRaw ? JSON.parse(linksRaw) : []; } catch { parsedLinks = []; }
  const cleanLinks = parsedLinks.filter((l) => l.url?.trim());
