...
import { getSocialLinkConfig, filterLinksByConfig } from "@/lib/queries/platform-settings";
...
function getLinkIcon(label: string): React.ReactNode {
  ...
  if (l.includes("instagram")) return <IgIcon />;
  if (l.includes("whatsapp"))  return <WaIcon />;
  if (l.includes("youtube"))   return <YtIcon />;
  return <WebIcon />;
}
...
export default async function CreatorProfilePage({
  ...
  const [portfolioItems, completedCount, responseTime, socialLinkConfig] = await Promise.all([
  ...
  // All links the creator has saved (collected regardless of admin config)
  // Merge legacy individual URL columns with the newer links JSON array,
  // so creators who saved via the old fields also get their links shown.
  const legacyLinks: import("@/lib/types").CreatorLink[] = [];
  if (creator.instagram_url) legacyLinks.push({ label: "Instagram", url: creator.instagram_url });
  if (creator.website_url)   legacyLinks.push({ label: "Website",   url: creator.website_url });
  if (creator.youtube_url)   legacyLinks.push({ label: "YouTube",   url: creator.youtube_url });

  const existingLabels = new Set((creator.links ?? []).map(l => l.label.toLowerCase()));
  const mergedLegacy = legacyLinks.filter(l => !existingLabels.has(l.label.toLowerCase()));
  const allLinks = [...(creator.links ?? []), ...mergedLegacy];

  // Only show links that admin has activated — this is filtered per admin config
  const visibleLinks = filterLinksByConfig(allLinks, socialLinkConfig);
  const hasVisibleLinks = visibleLinks.length > 0;

  // Gate: show links only to logged-in users
  const showLockedPlaceholder = hasVisibleLinks && !isLoggedIn;
  const showLinks = hasVisibleLinks && isLoggedIn;
              ...
              {showLinks && (
                <div className="cp-social-links">
                  {visibleLinks.map((link, i) => (
                    <SocialPill key={i} href={link.url} label={link.label} />
                  ))}
                </div>
              )}
          ...
          <section className="cp-footer-cta">
            <div className="cp-footer-cta__inner">
              ...
              <div className="cp-footer-cta__contact-options">
                ...
                {/* WhatsApp shortcut in footer — shown to logged-in users if they have a WA link */}
                {isLoggedIn && (() => {
                  const waLink = visibleLinks.find(l => l.label.toLowerCase().includes("whatsapp"));
                  return waLink ? (
                    <a href={waLink.url} target="_blank" rel="noopener noreferrer" className="cp-footer-cta__whatsapp">
                      <WaIcon />
                      Chat on WhatsApp
                    </a>
                  ) : null;
              ...
              {isLoggedIn && visibleLinks.length > 0 && (
                <div className="cp-footer-social">
                  {visibleLinks.map((link, i) => (
                    <a key={i} href={link.url} target="_blank" rel="noopener noreferrer" className="cp-footer-social__link" aria-label={link.label}>
